...
import './p5.Renderer';
import { Point, PrimitiveToContext2DConverter } from './shape/2d_shape';
 ...
 */
  ...
  endShape(
    ...
    // let v;
    // let i;
    // if (!this._clipping) this.drawingContext.beginPath(); // this one
    ...
    //   }
    // }
    this._doFillStrokeClose(closeShape); // this one ?
    ...
    shape.contours.forEach(contour => {
      ...
      contour.primitives.forEach(primitive => {
        if (primitive instanceof Point) {
          ...
          let v = primitive.vertex2d;
          console.log('v', v);
          if (this._doStroke) {
            this._pInst.stroke(v[6]);
          }
          this._pInst.point(v[0], v[1]);
          // primitive.accept(converter, this); // Visit each primitive with the converter
        }
